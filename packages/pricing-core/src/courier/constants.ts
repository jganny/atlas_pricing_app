...
export const SERVICE_LEVELS: Record<
  CourierServiceKey,
  { label: string; factor: number; transit: string }
> = {
...
export const CARRIER_FACTORS: Record<string, number> = {
  dhl: 1.05,
  fedex: 1.08,
  ups: 1.06,
  aramex: 0.92,
...
export const ZONE_RATES: Record<
  number,
  { min: number; breaks: Array<{ w: number; r: number }> }
> = {
...
export function roundChargeableKg(w: number): number {
  if (w <= 0) return 0;
  return Math.ceil(w * 2) / 2;
}

export function getCourierZone(originCountry: string, destCountry: string): number {
...
export function getRatePerKg(chargeableKg: number, zone: number) {
...
export function summarizeCourierPackages(
  packages: CourierPackageLine[],
  dimUnit: CourierDimUnit = "cm",
) {
  ...
  const oversized = lines.some(
    (p) => Math.max(p.l || 0, p.w || 0, p.h || 0) > 120 || (p.l || 0) + (p.w || 0) + (p.h || 0) > 300,
  );
...
export function convertFromUsd(
  ...
  if (currency === "INR") return amountUsd * rates.USD_TO_INR;
  if (currency === "EUR") return amountUsd / rates.EUR_TO_USD;
  if (currency === "GBP") return amountUsd / rates.GBP_TO_USD;
  if (currency === "AED") return amountUsd / rates.USD_TO_AED;
  if (currency === "SGD") return amountUsd / rates.USD_TO_SGD;
  if (currency === "AUD") return amountUsd / rates.USD_TO_AUD;
  if (currency === "CNY") return amountUsd / rates.USD_TO_CNY;
