...
export function calculateAirFreight(input: AirFreightInput): AirFreightResult {
  ...
  const { usedBreak, activeRate, activeBuyRate, usingBuyFallback } = selectActiveBreak(
    chargeable,
    input.breaks,
  );

  let baseFreightSell =
    tariffsEnabled && weightBreaksEnabled ? chargeable * activeRate : 0;
  let baseFreightBuy =
    tariffsEnabled && weightBreaksEnabled ? chargeable * activeBuyRate : 0;
