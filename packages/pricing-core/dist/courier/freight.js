import { CARRIER_FACTORS, COURIER_CARRIERS, SERVICE_LEVELS, convertFromUsd, getCourierZone, getRatePerKg, summarizeCourierPackages, } from "./constants.js";
function buildCarrierQuotes(chargeableKg, zone, serviceKey, currency, marginPct) {
...
function calcSurcharges(baseFreight, input) {
...
export function calculateCourierFreight(input) {
    const { lines, chargeableKg, oversized } = summarizeCourierPackages(input.packages, input.dimUnit);
    const zone = getCourierZone(input.originCountry, input.destCountry);
    const quotes = buildCarrierQuotes(chargeableKg, zone, input.service, input.currency, input.marginPct);
    ...
    const grossProfit = (chosen ? chosen.sellLocal - chosen.buyLocal : 0) +
