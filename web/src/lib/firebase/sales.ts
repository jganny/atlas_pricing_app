...
function mapLead(id: string, data: Record<string, unknown>): SalesLead {
  ...
  return {
    ...
    email: data.email ? String(data.email) : data.contactEmail ? String(data.contactEmail) : "",
    phone: data.phone ? String(data.phone) : data.contactPhone ? String(data.contactPhone) : "",
    ...
    owner: data.owner ? String(data.owner) : data.assignedTo ? String(data.assignedTo) : "",
